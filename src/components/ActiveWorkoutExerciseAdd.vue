<template>
    ...
    <div class='set-reps field mb-4 col-12 pl-0 pr-0'>
        ...
        <div class='flex field mb-4'>
            <InputNumber type="number" id="set-number" v-model="draftExercise.targetSets" :min="1" :step="1" pattern="\d*"
                showButtons />
        ...
        <ul class="exercise-sets">
            <li v-for="set in draftExercise.sets" :key="set">
                ...
                <InputNumber type='number' id="set-rep-number" v-model="set.target_reps" :min="1" :step="1" pattern="\d*" showButtons />
            </li>
        </ul>
    </div>
    <div class="surface-border border-top-1 opacity-50 mb-3 col-12"></div>
